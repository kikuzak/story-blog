...
const prefecture = await OldPrefectureLogic.getById(id);
...
const load = async ($state: any) => {
    try {
        const res = await ArticleViewLogic.getMutiByOldPrefectureAndPage(id, page, numPerPage);
        if (!res.value) {
